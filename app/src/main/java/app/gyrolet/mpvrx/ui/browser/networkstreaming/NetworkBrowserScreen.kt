...
import app.gyrolet.mpvrx.domain.network.isAudioFile
import app.gyrolet.mpvrx.domain.network.isVideoFile
import app.gyrolet.mpvrx.domain.network.isPlayableMediaFile
...
private fun NetworkBrowserContent(
  ...
  when {
    ...
    files.isEmpty() -> {
      ...
      val mediaFiles = files.filter { !it.isDirectory && it.isPlayableMediaFile() }
      val networkListState = LazyListState()

      // Only show scrollbar if list has more than 20 items (folders + media)
      val hasEnoughItems = (folders.size + mediaFiles.size) > 20
      ...
      PullRefreshBox(
        ...
        val scrollbarLabels = remember(folders, mediaFiles) {
          buildList<String?> {
