...
import app.gyrolet.mpvrx.domain.network.isM3uFile
...
class NetworkBrowserViewModel(
  ...
  private fun playVideoInternal(
    ...
    val resolvedMime = file.mimeType
      ?: FileTypeUtils.getMimeTypeFromExtension(file.name.substringAfterLast('.', ""))
      .takeIf { it != "video/*" }
  ...
  private fun createPlayableNetworkUri(
    ...
    return if (useProxy) {
      ...
      val resolvedMime = file.mimeType
        ?: FileTypeUtils.getMimeTypeFromExtension(ext).takeIf { it != "video/*" }
        ?: if (file.isAudioFile()) "audio/mpeg" else "video/mp4"
