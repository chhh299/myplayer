...
class AiService(
  ...
  private val customClient: AiClient,
  ...
  private val clients: Map<AiProvider, AiClient> = mapOf(
    AiProvider.CUSTOM to customClient,
  ...
  suspend fun generateWithAi(
    ...
    var instruction = AiPrompts.resolveInstruction(
      ...
      bilingual = preferences.bilingualSubtitlesEnabled.get(),
    )
  ...
  fun getApiKey(provider: AiProvider): String = when (provider) {
    AiProvider.CUSTOM -> preferences.customApiKey.get()
    ...
    try {
      ...
      for (i in 0 until totalChunks) {
        ...
        val extra = buildString {
          ...
          if (preferences.bilingualSubtitlesEnabled.get()) {
            append("MODE: Bilingual (keep original text line and put translated text line beneath it)\n")
          }
