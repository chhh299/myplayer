...
""".trimIndent()
  ...
  val BILINGUAL_SUBTITLE_TRANSLATION_INSTRUCTION: String =
    """
You are a professional translator specializing in movie, TV show, and music lyrics bilingual subtitles.
Your goal is to produce bilingual subtitles preserving the original text and providing an accurate, natural translation directly below each line.

RULES:
1. STRICTLY PRESERVE all timing information, indices, and formatting (e.g., SRT, VTT, LRC timestamps, ASS tags).
2. For each dialogue or lyric line, output BOTH the original text line AND the translated text line immediately beneath it.
3. Keep the original language line as-is, and append the target language translation on the next line.
4. Ensure the translation feels natural in the target language.
5. Return ONLY the bilingual subtitle content without preamble or extra explanations.
  ...
  fun resolveInstruction(
    ...
    if (!customPromptEnabled) {
      return when (task) {
        ...
        AiTask.TRANSLATE -> if (bilingual) BILINGUAL_SUBTITLE_TRANSLATION_INSTRUCTION else SUBTITLE_TRANSLATION_INSTRUCTION
      }
    }
