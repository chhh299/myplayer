...
import app.gyrolet.mpvrx.domain.lyrics.LrcParser
import app.gyrolet.mpvrx.domain.lyrics.LyricsDocument
...
object SubtitleOps : KoinComponent {
  ...
  suspend fun autoloadSubtitles(
    ...
    onLyricsLoaded: ((LyricsDocument) -> Unit)? = null,
    onCoverLoaded: ((String) -> Unit)? = null,
    ...
    try {
      ...
      if (networkConnectionId != -1L) {
        ...
        autoloadNetworkFileSubtitles(videoFilePath, videoFileName, networkConnectionId, onLyricsLoaded, onCoverLoaded)
        return@withContext
      }
      ...
      if (isNetworkStream) {
        ...
        autoloadLocalSubtitles(videoFilePath, videoFileName, onLyricsLoaded, onCoverLoaded)
      }
    } catch (e: Exception) {
  ...
  private suspend fun autoloadNetworkFileSubtitles(
    ...
    try {
      ...
      val coverFile = files.firstOrNull { file ->
        !file.isDirectory && file.name.lowercase() in setOf(
          "cover.jpg", "cover.png", "cover.jpeg",
          "folder.jpg", "folder.png", "album.jpg", "front.jpg",
        )
      }
      if (coverFile != null && onCoverLoaded != null) {
        val streamId = "cover_${networkConnectionId}_${coverFile.name.replace(" ", "_")}"
        val proxyUrl = NetworkStreamingProxy.getInstance().registerStream(
          streamId = streamId,
          connection = connection,
          filePath = coverFile.path,
          fileSize = coverFile.size,
          mimeType = if (coverFile.name.endsWith(".png", true)) "image/png" else "image/jpeg",
        )
        withContext(Dispatchers.Main) {
          onCoverLoaded(proxyUrl)
        }
      }
      ...
      // If there's an LRC/KRC lyric file, parse and emit to onLyricsLoaded
      val lrcSubtitle = subtitles.firstOrNull { file ->
        val ext = file.name.substringAfterLast('.', "").lowercase()
        ext == "lrc" || ext == "krc"
      }
      if (lrcSubtitle != null && onLyricsLoaded != null) {
        val client = NetworkClientFactory.createClient(connection)
        val lrcText = runCatching {
          client.connect().getOrThrow()
          client.getFileStream(lrcSubtitle.path).getOrThrow().bufferedReader(Charsets.UTF_8).use { it.readText() }
        }.also { runCatching { client.disconnect() } }.getOrNull()

        if (!lrcText.isNullOrBlank()) {
          val doc = LrcParser.parse(lrcText)
          withContext(Dispatchers.Main) {
            onLyricsLoaded(doc)
          }
        }
  ...
  private suspend fun autoloadLocalSubtitles(
    ...
    // Check for cover image in the same directory
    val coverFile = videoDirectory.listFiles()?.firstOrNull { file ->
    ...
    if (coverFile != null && onCoverLoaded != null) {
      withContext(Dispatchers.Main) {
        onCoverLoaded(coverFile.absolutePath)
      }
    }
    ...
    // If there's an LRC/KRC lyric file, parse and emit to onLyricsLoaded
    val lrcFile = subtitles.firstOrNull { file ->
      val ext = file.extension.lowercase()
      ext == "lrc" || ext == "krc"
    }
    if (lrcFile != null && onLyricsLoaded != null) {
      val lrcText = runCatching { lrcFile.readText(Charsets.UTF_8) }.getOrNull()
      if (!lrcText.isNullOrBlank()) {
        val doc = LrcParser.parse(lrcText)
        withContext(Dispatchers.Main) {
          onLyricsLoaded(doc)
